...
def bezierparameterize(bt):
    # parametric bezier
    x0 = bt.b0.x
    y0 = bt.b0.y
    cx = 3 * (bt.b1.x - x0)
    bx = 3 * (bt.b2.x - bt.b1.x) - cx
    ax = bt.b3.x - x0 - cx - bx
    cy = 3 * (bt.b1.y - y0)
    by = 3 * (bt.b2.y - bt.b1.y) - cy
    ay = bt.b3.y - y0 - cy - by
...
def linebezierintersect(lt, bt):
    ...
    ax, ay, bx, by, cx, cy, x0, y0 = bezierparameterize(bt)
    ...
    for i in roots:
        ...
        if type(i) is not complex and 0 <= i <= 1:
            retval.append(bezierpointatt(bt, i))
    return retval

# def bezierpointatt(((bx0, by0), (bx1, by1), (bx2, by2), (bx3, by3)), t):
def bezierpointatt(bt, t):
    ax, ay, bx, by, cx, cy, x0, y0 = bezierparameterize(bt)
...
def tpoint(b1, b2, t):
    return b1[0] + t * (b2[0] - b1[0]), b1[1] + t * (b2[1] - b1[1])


def beziersplitatt(bt, t):
    m1 = tpoint((bt.b0.x, bt.b0.y), (bt.b1.x, bt.b1.y), t)
    m2 = tpoint((bt.b1.x, bt.b1.y), (bt.b2.x, bt.b2.y), t)
    m3 = tpoint((bt.b2.x, bt.b2.y), (bt.b3.x, bt.b3.y), t)
    m4 = tpoint(m1, m2, t)
    m5 = tpoint(m2, m3, t)
    m = tpoint(m4, m5, t)

    return ((bt.b0.x, bt.b0.y), m1, m4, m), (m, m5, m3, (bt.b3.x, bt.b3.y))
...
def bezierlengthSimpson(bt, tolerance=0.001):
...
def beziertatlength(bt, l=0.5, tolerance=0.001):
    global balfax, balfbx, balfcx, balfay, balfby, balfcy
    ax, ay, bx, by, cx, cy, x0, y0 = bezierparameterize(bt)
    balfax, balfbx, balfcx, balfay, balfby, balfcy = 3 * ax, 2 * bx, cx, 3 * ay, 2 * by, cy
    t = 1.0
